...
data_class_b <- tibble(
  ...
  y = -2 * x + 5 + rnorm(n_obs, mean = 2, sd = 1)
) %>%
  mutate(classe = "B")
...
modelo_logistico_simples <- logistic_reg() %>%
  ...
  fit(classe ~ x + y, data = dados_simulados)

# 3. Extrair os coeficientes para a reta de classificação
...
# 4. Visualizar os dados e a reta de classificação
ggplot(dados_simulados, aes(x = x, y = y, color = classe)) +
  geom_point(size = 3, alpha = 0.7) +
...
library(rpart.plot)
rpart.plot(tree_fit$fit$fit, roundint = FALSE)
```
...
# Realizando a PCA
# Usamos apenas as colunas numéricas que serão usadas para a modelagem
...
# Fazendo as previsões para cada ponto na grade
grid_preds <- predict(linear_fit, new_data = grid, type = "class") %>%
  bind_cols(grid)

# Plotando as fronteiras de decisão e os dados originais
ggplot() +
  ...
  labs(
    title = "Fronteiras de Decisão do Modelo Linear (PCA + Regressão Logística)",
